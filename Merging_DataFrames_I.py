...
Hint
Use pd.read_csv() to read file_path into a DataFrame. Be sure to save the result to medals_dict[year].
Use [['Athlete', 'NOC', 'Medal']] on medals_dict[year] to extract the relevant columns.
Assign year to medals_dict[year]['Edition'] to create the new column whose entries are all year.
Use pd.concat() with medals_dict and ignore_index=True as arguments to create medals.'''
# Import pandas
import pandas as pd
...
Hint
To create the Series, assign influence['Change'] to change.
To create the bar plot, use the .plot() method on change with kind='bar' as the argument.
Follow the steps described in the instructions to customize the bar plot. Apply the relevant methods on the object ax.
Be sure to end your plotting statements with plt.show() to display the plot.'''
# Import pyplot
